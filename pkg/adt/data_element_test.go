...
func TestGetTypeInfoUsesDataElementDocument(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			// 合并后 GetTypeInfo 同时填充两侧字段集：Domain（fix-153 语义）
			// 与 TypeKind/DomainName（原 typeinfo 语义，predefined 时
			// DomainName 保持空）。
			want:     TypeInfo{Name: "ZDEMO_CODE", Type: "CHAR", Description: "Code & description", Length: 18, Decimals: 0, TypeKind: "predefinedAbapType"},
		...
		},
		{
			...
			want:     TypeInfo{Name: "/DEMO/AMOUNT", Type: "DEC", Domain: "ZDEMO_AMOUNT_DOMAIN", Description: "Amount", Length: 11, Decimals: 2, TypeKind: "domain", DomainName: "ZDEMO_AMOUNT_DOMAIN"},
